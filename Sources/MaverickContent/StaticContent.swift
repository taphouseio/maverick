...
public enum StaticContentError: Error, Equatable {
...
public struct StaticContentConfiguration: Sendable {
    ...
    public init(
        ...
        guard normalizedRoutePath.allSatisfy(Self.isSafePathComponent),
            ...
            throw StaticContentError.invalidConfiguration("Static content route and template configuration is invalid.")
        }
...
public struct StaticContentPageContext: Encodable, Equatable, Sendable {
...
public struct StaticContentRenderTarget: Sendable {
    public let template: String
    public let context: StaticContentPageContext

    public init(template: String, context: StaticContentPageContext) {
        self.template = template
        self.context = context
    }
}

public struct StaticContentAsset: Sendable {
...
public actor StaticContentStore {
    ...
    private let configuration: StaticContentConfiguration
    private var snapshot: Snapshot?
    private var failedFingerprint: String?

    public init(configuration: StaticContentConfiguration) {
        self.configuration = configuration
    }

    public func renderTarget(for path: String) throws -> StaticContentRenderTarget {
        ...
        guard let page = snapshot.pages[normalizedPath] else {
            throw StaticContentError.notFound(normalizedPath)
        }
        ...
        return StaticContentRenderTarget(template: template, context: context)
    }

    public func asset(pagePath: String, assetPath: String) throws -> StaticContentAsset {
        let normalizedPagePath = try normalizedContentPath(pagePath)
        let snapshot = try currentSnapshot()
        guard let page = snapshot.pages[normalizedPagePath] else {
            throw StaticContentError.notFound(normalizedPagePath)
        }
        ...
        guard isDescendant(fileURL, of: assetsRoot),
            ...
            throw StaticContentError.invalidAsset(assetPath)
        }
    ...
    private func buildSnapshot(fingerprint: String) throws -> Snapshot {
        ...
        guard FileManager.default.fileExists(atPath: root.path) else {
            throw StaticContentError.invalidContent("Static content root does not exist: \(root.path)")
        }
        ...
        if let bundleURL = configuration.bundleURL {
            ...
            guard let enumerator = FileManager.default.enumerator(
                ...
                throw StaticContentError.invalidContent("Unable to enumerate static content.")
            }
        ...
        for candidate in candidates {
            ...
            guard isDescendant(candidate, of: root),
                ...
                throw StaticContentError.invalidContent("Bundle escapes the static content root: \(candidate.path)")
            }
            ...
            guard !bundle.metadata.title.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty else {
                throw StaticContentError.invalidContent("Bundle has an empty title: \(candidate.path)")
            }
            if bundle.metadata.draft && !configuration.includeDrafts { continue }
            guard pendingPages[contentPath] == nil else {
                throw StaticContentError.invalidContent("Duplicate static content path: \(contentPath)")
            }
        ...
        for page in pendingPages.values {
            ...
            do {
                ...
                case .invalidAssetSource(let source):
                    throw StaticContentError.invalidAsset(source)
                }
            }
    ...
    private func makePage(for page: PageRecord, from snapshot: Snapshot) -> StaticContentPageContext {
        ...
        return StaticContentPageContext(
            path: page.path,
    ...
    private static func normalizedLinkPath(_ components: [String]) throws -> String {
        ...
        for component in components {
            ...
            case "", ".": continue
            case "..":
                guard !normalized.isEmpty else { throw StaticContentError.invalidContent("Internal link escapes the content root.") }
                normalized.removeLast()
            default:
                guard !component.contains("\\") else { throw StaticContentError.invalidContent("Invalid internal link.") }
    ...
    private static func contentURL(configuration: StaticContentConfiguration, path: String) -> String {
    ...
    private func contentFingerprint() throws -> String {
        ...
        guard let enumerator = FileManager.default.enumerator(
            ...
            throw StaticContentError.invalidContent("Unable to fingerprint static content.")
        }
    ...
    private func relativePath(of url: URL, from root: URL) throws -> String {
        ...
        guard url.standardizedFileURL.path.hasPrefix(rootPath) else {
            throw StaticContentError.invalidContent("Bundle is outside the configured content root.")
        }
    ...
    private func contentPath(forBundleRelativePath path: String) throws -> String {
        ...
        guard let last = components.last, last.hasSuffix(".textbundle") else {
            throw StaticContentError.invalidContent("Invalid textbundle path: \(path)")
        }
    ...
    private func normalizedContentPath(_ path: String) throws -> String {
        ...
        guard components.allSatisfy({ $0 != "." && $0 != ".." && !$0.contains("\\") }) else {
            throw StaticContentError.invalidContent("Invalid static content path: \(path)")
        }
    ...
    private static func normalizedAssetComponents(_ path: String) throws -> [String] {
        ...
        guard !components.isEmpty, components.allSatisfy({ $0 != "." && $0 != ".." && !$0.contains("\\") }) else {
            throw StaticContentError.invalidAsset(path)
        }
        return components
    }
...
public struct StaticContentRouteCollection<Context: Encodable & Sendable>: RouteCollection, Sendable {
    private let configuration: StaticContentConfiguration
    private let store: StaticContentStore
    private let makeContext: @Sendable (StaticContentPageContext) -> Context

    public init(
        configuration: StaticContentConfiguration,
        store: StaticContentStore? = nil,
        makeContext: @escaping @Sendable (StaticContentPageContext) -> Context
    ) {
        self.configuration = configuration
        self.store = store ?? StaticContentStore(configuration: configuration)
...
private extension StaticContentError {
    var abort: Abort {
        switch self {
